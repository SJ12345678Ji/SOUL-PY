...
def approve_or_disapprove_user(message):
    ...
    if not is_admin:
        bot.send_message(chat_id, "*BHENCHOD APNE BAP SE PAHELE AUTHORIZATION LETTER LEKE AA FIR CHUDAI KARNA😂*", parse_mode='Markdown')
        return
    ...
    if action == '/approve':
        if plan == 1:  # Chudai 🚀
            if users_collection.count_documents({"plan": 1}) >= 99:
                ...
                return
        elif plan == 2:  # Instant Chudai 🚀
            if users_collection.count_documents({"plan": 2}) >= 499:
                bot.send_message(chat_id, "*Approval failed: Instant Chudai 🚀 limit reached (499 users).*", parse_mode='Markdown')
                return
...
def attack_command(message):
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "BHENCHOD APNE BAP SE PAHELE AUTHORIZATION LETTER LEKE AA FIR CHUDAI KARNA😂")
            return
        ...
        if user_data['plan'] == 2 and users_collection.count_documents({"plan": 2}) > 499:
            bot.send_message(chat_id, "Your Instant Chudai 🚀 is currently not available due to limit reached.")
            return
...
def attack_command(message):
    ...
    try:
        ...
        if user_data['plan'] == 2 and users_collection.count_documents({"plan": 2}) > 499:
            bot.send_message(chat_id, "*Your Instant Chudai 🚀 is currently not available due to limit reached.*", parse_mode='Markdown')
            return
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*SEX STARTED 💥\n\nBOOBS SIZE: {target_ip}\nCHUT COLOR: {target_port}\nKITNE DER TK TIKA RHEGA: {duration}*", parse_mode='Markdown')
    except Exception as e:
...
def send_welcome(message):
    ...
    btn2 = KeyboardButton("Instant Chudai 🚀")
...
def handle_message(message):
    ...
    elif message.text == "Instant Chudai 🚀":
        bot.reply_to(message, "*Instant++ Plan selected*", parse_mode='Markdown')
        attack_command(message)
    elif message.text == "Canary Download✔️":
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download = @Soulbgmix*", parse_mode='Markdown')
